...
def get_data():
    ...
    transformed_data = (
        data.melt(id_vars=data.columns[:14], value_vars=data.columns[14:])
        .dropna(subset="value")
    ...
    transformed_data["country"] = transformed_data["country"].apply(
...
def calculate_statistics_jr_scale(data: pd.DataFrame, query: str | None = None):
    ...
    indexed_data = indexed_data[
        indexed_data["value"].apply(lambda x: isinstance(x, (float, int)))
    ]
    ...
    statistics_data = pd.merge(
        statistics_data,
        indexed_data[indexed_data["value"] == 3]
        .rename(columns={"value": "JR"})
        .groupby(["attribute"])
        .count(),
        left_index=True,
        right_index=True,
...
def calculate_statistics_regular_scale(data: pd.DataFrame, query: str | None = None):
    ...
    statistics_data = pd.merge(
        statistics_data,
        indexed_data[indexed_data["value"] == 5]
        .rename(columns={"value": "TB"})
        .groupby(["attribute"])
        .count(),
        left_index=True,
        right_index=True,
    ...
    statistics_data = pd.merge(
        statistics_data,
        indexed_data[indexed_data["value"].isin([4, 5])]
        .rename(columns={"value": "T2B"})
        .groupby(["attribute"])
        .count(),
        left_index=True,
        right_index=True,
    ...
    statistics_data = pd.merge(
        statistics_data,
        indexed_data[indexed_data["value"].isin([1, 2])]
        .rename(columns={"value": "B2B"})
        .groupby(["attribute"])
        .count(),
        left_index=True,
        right_index=True,
