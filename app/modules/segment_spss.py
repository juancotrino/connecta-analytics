...
def process_chi2(
    ...
    filtered_variables = [
        variable for variable, scale in variables_data.items() if len(scale) == 5
    ]
    filtered_variables_data = {
        k: v for k, v in variables_data.items() if k in filtered_variables
    }

    just_right_variables = [
        code
        for code, tag in filtered_variables_data.items()
        if "just" in "".join(tag.values()).lower()
    ]
    ...
    inverted_variables = [
        ...
        and (code not in just_right_variables)
    ]

    filtered_data: pd.DataFrame = data[
        data.columns[data.columns.isin(filtered_variables)]
    ]
    filtered_data_bool = filtered_data.copy()

    variables_scale_info = pd.DataFrame(
        list(study_metadata.column_names_to_labels.items()), columns=["code", "text"]
    ).replace({None: np.nan})
    ...
    variables_scale_info["scale_type"] = np.where(
        ...
        "just_right",
        np.where(
            variables_scale_info["code"].isin(inverted_variables), "inverted", "regular"
        ),
    )
    variables_scale_info["scale_type"] = variables_scale_info["scale_type"].replace(
        {None: np.nan}
    )

    for column in filtered_data:
        scale_type = variables_scale_info[variables_scale_info["code"] == column][
            "scale_type"
        ].values[0]
        filtered_data_bool.loc[:, column] = filtered_data.loc[:, column].map(
            scales[chi2_mode][scale_type]
        )
    ...
    for column in filtered_data_bool.columns:
        ...
        if column != cross_variable:
            chi2, p_value, cramer_v = chi_square_test(
                filtered_data_bool[cross_variable],
                filtered_data_bool[column],
                correction,
            )
            results[column] = {
                "Chi-square": chi2,
    ...
    if not results_df.empty:
        return results_df[["text", "Cramer's V", "P-value", "scale_type"]].sort_values(
            by=["Cramer's V"], ascending=True
        )
    else:
        return pd.DataFrame()


def get_correlation_data(file_path: str, correlation_variables: list[str]):
    data: pd.DataFrame = pyreadstat.read_sav(file_path, apply_value_formats=False)[0]
...
def process_correlation(data: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for col1 in data.columns:
        ...
        for col2 in data.columns:
            if (
                col2 == col1 or data[col2].notnull().sum() == 0
            ):  # Skip column if it has no non-null values or if it's the same column
...
def create_chart(worksheet, source_chart, dataframe, chart_title, chart_destination):
    ...
    data = Reference(
        worksheet, min_col=2, min_row=2, max_row=len(dataframe) + 1, max_col=2
    )
...
def format_ws(sheet):
    ...
    for row in sheet.iter_rows(
        min_row=2, min_col=2, max_row=max_row, max_col=max_column
    ):
        for cell in row:
    ...
    for i in range(len(df) - 1, -1, -1):
        ...
        if not condition is None and "#" in condition:
            ...
            for comb in combinations:
                df = pd.concat(
                    [df.iloc[: i + 1], pd.DataFrame([row_original]), df.iloc[i + 1 :]]
...
def segment_spss(
    ...
    if jobs["cross_variable"].any():
        ...
        chi2_xlsx_file_name = (
            f"chi2_{spss_file.name.split('.')[0].replace('Base ', '')}.xlsx"
        )
    ...
    if jobs["correlation_variables"].any():
        ...
        # Create a new Excel file
        corr_xlsx_file_name = (
            f"correlations_{spss_file.name.split('.')[0].replace('Base ', '')}.xlsx"
        )
