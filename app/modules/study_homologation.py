...
def check_sharepoint_folder_existance(id_study_name: str, sharepoint: SharePoint):
    studies_in_sharepoint = sharepoint.list_folders(
        "Documentos compartidos/estudios_externos"
    )
...
def variables_validation(selected_variables: dict, metadata_df: pd.DataFrame):
    ...
    missing_values = {
        ...
        if v not in [value.split(".")[0] for value in metadata_df.index.values]
    }
    if missing_values:
...
def get_scales_data(study_number: int, study_data: pd.DataFrame, metadata):
    scales_data_list = [
        pd.DataFrame(
            {
                ...
                "question": list(
                    study_data[study_data["question_code"] == variable.split(".")[0]][
                        "question"
                    ].values
                )
                * len(labels),
        ...
        if (
            ...
            or (variable in study_data["question_code"].tolist())
        )
        and ("justo" not in " ".join(labels.values()).lower())
    ]

    if scales_data_list:
        scales_data = pd.concat(scales_data_list).reset_index(drop=True)

        count_scales_data = scales_data.groupby(
            ["study_number", "question_code", "question"]
        ).count()
        unique_count_scales_data = count_scales_data.reset_index()[
            "answer_code"
        ].unique()
        print(
            f"Unique values of answer options length for regular questions (should only be 5): {', '.join(unique_count_scales_data.astype(str))}"
        )
...
def get_jr_scales_data(study_number: int, study_data: pd.DataFrame, metadata):
    jr_scales_data_list = [
        ...
        and ("justo" in " ".join(labels.values()).lower())
    ]

    if jr_scales_data_list:
        jr_scales_data = pd.concat(jr_scales_data_list).reset_index(drop=True)

        count_jr_scales_data = jr_scales_data.groupby(
            ["study_number", "question_code", "question"]
        ).count()
        unique_count_jr_scales_data = count_jr_scales_data.reset_index()[
            "answer_code"
        ].unique()
        print(
            f"Unique values of answer options length for JR questions (should only be 5 or 3): {', '.join(unique_count_jr_scales_data.astype(str))}"
        )

        jr_scales_data["is_inverted"] = np.where(
            ~jr_scales_data["jr_option"].isna(), True, False
        )
        jr_scales_data["real_value"] = np.where(
            jr_scales_data["jr_option"] == jr_scales_data["answer_code"],
            3,
            np.where(
                jr_scales_data["is_inverted"] == False, jr_scales_data["answer_code"], 1
            ),
        ).astype(int)
...
def assign_real_values(df: pd.DataFrame, real_value_df: pd.DataFrame | None):
    ...
    df["value"] = np.where(
        df["value"] == df["answer_code"], df["real_value"], df["value"]
    ).astype(int)
...
def get_logs(study_info: dict):
    ...
    logs_demographics = logs_demographics_df.to_markdown(
        headers="keys", tablefmt="psql", index=False
    )
    logs_variable_mapping = logs_variable_mapping_df.to_markdown(
        headers="keys", tablefmt="psql", index=False
    )
...
def transponse_df(df: pd.DataFrame):
    ...
    transformed_data["category"] = transformed_data["category"].apply(
        lambda x: x.strip()
    )
    # transformed_data['gender'] = transformed_data['gender'].astype(str)
    transformed_data["age"] = transformed_data["age"].apply(
...
def get_current_studies(_bq: BigQueryClient):
    return _bq.fetch_data(
        ...
        SELECT DISTINCT
            study_number,
            country
        FROM `connecta-app-1.normas.estudios_externos`;
        """
    )
...
def check_study_existance_in_bq(study_number: int, country: str, studies: pd.DataFrame):
    study = studies[
        (studies["study_number"] == study_number) & (studies["country"] == country)
    ]
...
def load_to_bq(df: pd.DataFrame, bq: BigQueryClient, action: str = "load"):
    ...
    match action:
        case "update":
            bq.delete_data(
                """
                DELETE `connecta-app-1.normas.estudios_externos`
                WHERE study_number = {study_number}
                    AND country = '{country}';
                """.format(study_number=study_number, country=country)
            )
...
def process_study(spss_file_name: str, study_info: dict):
    ...
    metadata = pyreadstat.read_sav(spss_file_name, apply_value_formats=False)[1]
    ...
    survey_data_tags: pd.DataFrame = pyreadstat.read_sav(
        spss_file_name, apply_value_formats=True
    )[0].dropna(how="all")

    survey_data: pd.DataFrame = pyreadstat.read_sav(
        spss_file_name, apply_value_formats=False
    )[0].dropna(how="all")
    ...
    demographic_variables_codes = study_data[
        study_data["question"].isin(demographic_variables)
    ]["question_code"].to_list()

    pattern = study_info["sample_variable"]

    sample_columns = survey_data[
        survey_data.columns[survey_data.columns.str.contains(pattern, regex=True)]
    ].columns.tolist()

    survey_data.loc[:, demographic_variables_codes + sample_columns] = (
        survey_data_tags.loc[:, demographic_variables_codes + sample_columns]
    )
    ...
    for _, survey in survey_data.iterrows():
        ...
        if len(sample_columns) > 1 and all(test_samples_nan):
            print(
                "Inconsistency: There is more than one sample but there are no variables with pattern PXX.X or pXX.X or NXX.X"
            )
            break
        ...
        if all(test_samples_nan):
            ...
            answers["question_code"] = answers["question_code"].apply(
            ...
            samples = pd.DataFrame(
                {
                    "question_code": [1],
                    "sample": answers[answers["question_code"].str.contains(pattern)][
                        "answer"
                    ].values,
                    "sample_number": [1],
                }
            ...
            answers["sample_number"] = np.where(
                answers["question_code"].isin(study_data["question_code"]),
                answers["sample_number"],
                np.nan,
            )

            test_samples_nan = answers["sample_number"].isna()

            if len(sample_columns) > 1 and all(test_samples_nan):
                print(
                    "Inconsistency: (Second instance) There is more than one sample but there are no variables with pattern PXX.X or pXX.X or NXX.X"
                )
                break
            ...
            if all(test_samples_nan):
                ...
                samples = pd.DataFrame(
                    {"question_code": [1], "sample": [1], "sample_number": [1]}
                )

            # Get samples of this survey
            samples = (
                answers[answers["question_code"].str.contains(pattern, regex=True)]
                .reset_index(drop=True)
                .replace({0: np.nan})
            )
            ...
            samples["sample_number"] = (
                answers[
                    (~answers["sample_number"].isna()) & (~answers["answer"].isna())
                ]["sample_number"]
                .unique()
                .astype(int)
            )
        ...
        questions_with_sample = answers_merged[
            ~answers_merged["sample_number"].isna()
        ].reset_index(drop=True)
        questions_without_sample = answers_merged[
            answers_merged["sample_number"].isna()
        ].reset_index(drop=True)
        ...
        pivoted_answers = (
            questions_answers_merged.drop(columns="question_code")
            .pivot(columns="question", index="sample_number", values="answer")
            .reset_index()
        )

        pivoted_answers["sample_number"] = pivoted_answers["sample_number"].astype(int)

        pivoted_answers = samples.drop(columns="question_code").merge(
            pivoted_answers, on="sample_number"
        )
        ...
        pivoted_answers["sys_RespNum"] = (
            answers[answers["question_code"] == "sys_RespNum"]
            .reset_index(drop=True)["answer"]
            .astype(int)
            .loc[0]
        )
        ...
        final_data_template = pd.concat(
            [final_data_template, pivoted_answers]
        ).reset_index(drop=True)
    ...
    final_data_template = (
        final_data_template.replace({np.nan: None})
        .drop_duplicates()
        .reset_index(drop=True)
    )
