...
def main():
    ...
    with st.expander("Question Finder"):
        entryText = st.text_area(
            "Text Entry:",
            placeholder="Copy and paste the entire text of the questionnaire",
        )
    ...
    with st.expander("Generate Labels"):
        entryText = st.text_area(
            "Questions:", placeholder="Copy and paste the list of questions", height=200
        )
        entryText2 = st.text_area(
            "Labels SPSS:", placeholder="Copy and paste the label column from SPSS"
        )
    ...
    with st.expander("Get KPI's from Tables:"):
        ...
        with col1:
            ...
            tablas_xlsx = st.file_uploader(
                "Upload `.xlsx` file", type=["xlsx"], key="tablas_xlsx"
            )
        with col2:
            st.markdown("#### KPI's List")
            kpis_list_xlsx = st.file_uploader(
                "Upload `.xlsx` file", type=["xlsx"], key="kpis_list_xlsx"
            )

        btn_get_kpis = st.button("Get KPI's")
        if btn_get_kpis:
            with st.spinner("Get KPIs..."):
                results_tables_kpis, _ = get_kpis_tables(tablas_xlsx, kpis_list_xlsx)
                st.success("Tables with KPIs generate successfully.")
        try:
            try_download(
                "Download Tables with KPIs",
                results_tables_kpis,
                tablas_xlsx.name + "-KPIs",
                "xlsx",
            )
        except Exception:
            pass
    ...
    with st.expander("Get totals from Pretablas:"):
        pretabla_xlsx = st.file_uploader(
            "Upload `.xlsx` file", type=["xlsx"], key="pretabla_xlsx"
        )
    ...
    with st.expander("Compare 2 Bases SAV Files:"):
        col1, col2 = st.columns(2)
        with col1:
            comparebases1_sav = st.file_uploader(
                "Upload `.sav` file", type=["sav"], key="comparebases1_sav"
            )
        with col2:
            comparebases2_sav = st.file_uploader(
                "Upload `.sav` file", type=["sav"], key="comparebases2_sav"
            )
        btn_get_comparison = st.button("Get Comparison")
        if btn_get_comparison:
            with st.spinner("Get Comparison..."):
                results_totals2 = get_comparison_tables(
                    comparebases1_sav, comparebases2_sav
                )
        ...
        try:
            try_download(
                ...
                results_totals2,
                "comparison_tables-"
                + comparebases1_sav.name[:10]
                + "-"
                + comparebases2_sav.name[:10],
                "xlsx",
            )
        except Exception:
            pass
    ...
    with st.expander("Open-ended questions transformation"):
        ...
        open_ended_questions_xlsx = st.file_uploader(
            "Upload `.xlsx` file", type=["xlsx"], key="open_ended_questions_xlsx"
        )
        if open_ended_questions_xlsx:
            ...
            config = {
                "questions": st.column_config.TextColumn(
                    "Questions", width="medium", required=True
                ),
            }
        ...
        try:
            st.download_button(
                ...
                file_name="open_ended_transformed.xlsx",
        ...
        with st.form("open_ended_coded_transformation"):
            open_ended_questions_coded_xlsx = st.file_uploader(
                "Upload `.xlsx` file",
                type=["xlsx"],
                key="open_ended_questions_coded_xlsx",
            )
            closed_questions_db_sav = st.file_uploader(
                "Upload `.sav` file", type=["sav"], key="closed_questions_db_sav"
            )

            transform = st.form_submit_button("Transform")

            if (
                open_ended_questions_coded_xlsx
                and closed_questions_db_sav
                and transform
            ):
                temp_file_name_xlsx = get_temp_file(
                    open_ended_questions_coded_xlsx, ".xlsx"
                )
                temp_file_name_sav = get_temp_file(closed_questions_db_sav)

                final_df, metadata = generate_open_ended_db(
                    temp_file_name_xlsx, temp_file_name_sav
                )
        ...
        try:
            st.download_button(
                ...
                file_name="open_ended_coded_transformed.sav",
    ...
    with st.expander("Compare LC Files:"):
        col1, col2 = st.columns(2)
        with col1:
            comparelc1_xlsx = st.file_uploader(
                "Upload `.xlsx` file", type=["xlsx", "xlsm"], key="comparelc1_xlsx"
            )
        with col2:
            comparelc2_xlsx = st.file_uploader(
                "Upload `.xlsx` file", type=["xlsx", "xlsm"], key="comparelc2_xlsx"
            )
        ...
        try:
            try_download(
                "Download LC Comparison tables",
                results_comparelc,
                "lccomparison_tables-"
                + comparelc1_xlsx.name[:10]
                + "-"
                + comparelc2_xlsx.name[:10],
                "xlsx",
            )
        except Exception:
            pass
    ...
    with st.expander("SPSS split and join"):
        ...
        with st.form("split_spss_form"):
            split_file = st.file_uploader(
                "Upload `.sav` file", type=["sav"], key="split_file_sav"
            )
            number_of_records = st.number_input(
                "Number of records per chunk", step=1, min_value=1
            )
            ...
            if split_file and number_of_records and split_database:
                ...
                chunks, meta = split_sav_file_to_chunks(
                    temp_split_file, number_of_records
                )
        ...
        with st.form("join_spss_form"):
            ...
            original_db = st.file_uploader(
                "Upload `.sav` file", type=["sav"], key="join_original_db_sav"
            )
        ...
        try:
            try_download(
                "Download joined database", joined_database, "db_joined", "sav"
            )
        except Exception:
            pass
