...
def filter_files_by_roles(
    files_info: dict, user_roles: list
...
def main():
    ...
    if "connecta-field" not in st.session_state["roles"]:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["Studies viewer", "Create study", "Edit study", "File uploader"]
        )
    else:
    ...
    if "connecta-field" not in st.session_state["roles"]:
        with tab1:
            ...
            with bottom_menu[1]:
                total_pages = (
                    int(number_of_studies / batch_size)
                    if int(number_of_studies / batch_size) > 0
                    else 1
                )
            ...
            pagination.dataframe(
                ...
                column_config={
                    ...
                    for column in paginated_data.columns
                },
            ...
            with st.form("create_study"):
                ...
                if create_button:
                    with st.spinner("Creating study..."):
                        combinations = list(
                            itertools.product(methodologies, study_types, countries)
                        )
                        study_data = {
                            ...
                            "study_type": [
                                combination[1] for combination in combinations
                            ],
            ...
            if study_id:
                with st.form("edit_study"):
                    ...
                    study_name = st.text_input(
                        ...
                        disabled=True
                        if study_data["source"].values[0] == "app"
                        else False,
                    )
                    ...
                    value = st.number_input("Study value/price", value=value1, step=1)
                    ...
                    currency = st.selectbox(
                        ...
                        if study_data["currency"].values[0] in currencies
                        else 0,
                    )
                    ...
                    description = st.text_area(
                        "Description", value=study_data["description"].values[0]
                    )
                    ...
                    supervisor = st.selectbox(
                        ...
                        if study_data["supervisor"].values[0] in supervisors
                        else 0,
                        placeholder="Select consultant...",
                    )
                    ...
                    if edit_study:
                        with st.spinner("Updating study..."):
                            combinations = list(
                                itertools.product(
                                    methodologies, study_types_selected, countries
                                )
                            )
                            updated_study_data = {
                                ...
                                "description": [description] * len(combinations),
                                "country": [
                                    combination[2] for combination in combinations
                                ],
                                "client": [client] * len(combinations),
                                "value": [value] * len(combinations),
                                "currency": [currency] * len(combinations),
                        ...
                        if (
                            status == "En ejecución"
                            and study_data["status"].values[0] != "En ejecución"
                        ):
                            countries_folders = {}
                            for country in countries:
                                ...
                                base_path = (
                                    f"Documentos compartidos/estudios/{id_study_name}"
                                )
                                try:
    ...
    with tab4:
        ...
        if not accessible_files:
            st.warning("You have no permissions to upload files.")
        else:
            study_id = st.selectbox(
                "Study ID",
                options=reversed(sudies_ids),
                index=None,
                placeholder="Select study ID",
                key="study_id_file_uploader",
            )
            if study_id:
                study_data = get_study_data(study_id)
                studies_countries = (
                    study_data[study_data["study_id"] == study_id]["country"]
                    .sort_values()
                    .unique()
                )
                ...
                if len(studies_countries) > 1:
                    country = st.selectbox(
                        "Country",
                        options=studies_countries,
                        index=None,
                        placeholder="Select study country",
                        key="country_file_uploader",
                    )
                ...
                if country is not None:
                    ...
                    if study_id and country:
                        if len(list(set(specific_studies))) > 1:
                            specific_study = st.radio(
                                "Select study:", options=specific_studies, index=None
                            )
                        else:
                    ...
                    if study_id and country and specific_study:
                        ...
                        st.info(
                            f"Upload to `{specific_study}` study for country: {country}"
                        )
                        ...
                        title = st.selectbox(
                            ...
                            options=sorted(
                                [
                                    ...
                                    for file in accessible_files.keys()
                                ]
                            ),
                        )
                        ...
                        with st.form(f"upload_{title}_form"):
                            if file_info["acronym"] and file_info["file_type"]:
                                ...
                                upload_file = st.form_submit_button(
                                    f"Upload {title.replace('_', ' ')}"
                                )
                            else:
                                uploaded_file = st.file_uploader(
                        ...
                        if uploaded_file and upload_file:
                            ...
                            if file_info["acronym"] and file_info["file_type"]:
                                files = get_last_file_version_in_sharepoint(
                                    id_study_name, "estudios", file_path
                                )
                                files = [
                                    file
                                    for file in files
                                    if file_info["acronym"] in file
                                ]
                                if not files:
                            ...
                            with st.spinner(
                                f"Uploading {title.replace('_', ' ')} to Sharepoint..."
                            ):
                                try:
                                    upload_file_to_sharepoint(
                                    ...
                                    st.error(
                                        "There is no folder for this study in SharePoint."
                                    )
                            ...
                            try:
                                msteams_card_file_update(
                                    ms_teams_file_name, ms_teams_study_info
                                )
                            except Exception:
                                st.error(
                                    "There is no folder for this study in SharePoint."
